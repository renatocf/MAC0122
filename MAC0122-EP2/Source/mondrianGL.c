...
/* 
////////////////////////////////////////////////////////////////////// 
  2  MACROS E CONSTANTES e VARIAVEIS GLOBAIS
...
void carregaEPinta(Imagem *C, Imagem *R, Imagem *G, Imagem *B);
...
int main(int argc, char** argv)
{
  	/** ALOCAÇÃO DE VARIÁVEIS ****************************************/
		/** Variáveis do esqueleto-ep2gl.c ***************************/
		Imagem *imgC; 		/* Imagem cinza 						 */
	 	Imagem *imgR; 		/* Canal R da imagem colorida 			 */
  		...
  		int numRegioes;		/* 	Auxiliar para nº de regiões	achadas	 */
	...
	/** FUNDO E BORDAS ***********************************************/	
		imgC = criaImagem(altura, largura);
		pintaImagem(imgC, COR_FUNDO);
		desenhaBorda(imgC, COR_BORDA);
	
		
	/** DESENHO DE LINHAS ********************************************/
		for(atual = cab.prox; atual != NULL; atual = atual->prox)
			desenhaLinha(imgC, atual, COR_BORDA);
	
	
	/** SEGMENTAÇÃO DE REGIÕES ***************************************/
		numRegioes = segmentaRegioes(imgC, cabecas);
		
	
	/** CRIAÇÃO DOS CANAIS DE CORES **********************************/
		imgR = criaImagem(altura, largura); 
		copiaImagem(imgR, imgC);				/* Vermelho */
		
		imgG = criaImagem(altura, largura); 
		copiaImagem(imgG, imgC);				/* Verde */
		
		imgB = criaImagem(altura, largura); 
		copiaImagem(imgB, imgC);				/* Azul */
		
		
	/** PINTURA ******************************************************/
		for(i = 0; i < numRegioes; i++)
			pintaRegiao(&cabecas[i].cabpix, imgR, imgG, imgB, 
				CORES[i % NUM_CORES]);
	
	
	/** SALVAR IMAGEM ************************************************/
		salvaImagemRGB(argv[2], imgR, imgG, imgB);
...
float getPixel(Imagem *img, int x, int y)
	/* 	A função recebe o endereço de uma imagem e uma coordenada (x,y);
		Devolve o valor do pixel nesta posição */
{
...
int juntaPixels(Imagem *img, int x, int y, float corNova, CelPixel *cabeca)
	/* 	A função juntaPixels recebe uma imagem e 2 posições. Captura re-
		...
		'cabeça'. Devolve o total de pixels da região. */
{	
	/** ALOCAÇÃO DE VARIÁVEIS *****************************************/
		int r; /*contador*/
		CelPixel *nova;
	...
	/** NOVAS CHAMADAS RECURSIVAS *************************************/
		r += juntaPixels(img, x+1, y, corNova, cabeca); /* direita	*/
		r += juntaPixels(img, x, y+1, corNova, cabeca); /* baixo 	*/
		r += juntaPixels(img, x-1, y, corNova, cabeca); /* esquerda	*/
		r += juntaPixels(img, x, y-1, corNova, cabeca); /* cima 	*/
...
int  segmentaRegioes(Imagem *img, CelRegiao cabecas[MAX_REGIOES])
	/* 	A função recebe uma imagem img e um vetor de estruturas 
		'CelRegiao' com tamanho MAX_REGIOES. Percorre a imagem 
		até achar pixels com cor COR_FUNDO. Coloca em cada ele-
		mento do vetor uma lista ligada com cabeça corresponden-
		te ao conjunto de pixels da região da imagem onde está o
		pixel com cor COR_FUNDO. Devolve o número de regiões 
		encontradas. */
{
	int i, j;			
	int ultima = 0;		
...
void configuraParteGrafica(Imagem *R, Imagem *G, Imagem *B, int numRegioes, CelRegiao cabecas[MAX_REGIOES])
{
  /* salva as variaveis globais */
  gImgR = R;
  gImgG = G;
  gImgB = B;
  gNumRegioes = numRegioes;
  gCabecas = cabecas;

  /* GLUT */
  glutInitDisplayMode (GLUT_SINGLE | GLUT_RGB);
  glutInitWindowSize ( R->nC, R->nL );
  glutInitWindowPosition (100, 100);
  
  glutCreateWindow ("Pinta Mondrian");
...
/* Chamada quando uma tecla é apertada           
   ...
   t: troca uma cor
   s: salva a imagem exibida no arquivo saidaMondrian.ppm
*/
...
void myKeyboard (unsigned char key, int mx, int my)
{
  ...
  switch (key)
    ...
    case 't':
    case 'T':
      printf("Digite 2 inteiros para a regiao e cor: ");
      scanf("%d %d", &r, &c);
      if (r < 0) exit(0);
      if (r >= 0 && r < gNumRegioes)
	pintaRegiao(gCabecas[r].cabpix.prox, gImgR, gImgG, gImgB, CORES[c % NUM_CORES]);
      else
	printf("Regiao %d invalida\n", r);
      glutPostRedisplay(); 
      break;
      
    case 's':
    case 'S':
      salvaImagemRGB ("saidaMondrian.ppm", gImgR, gImgG, gImgB);
      printf("\nImagem salva no arquivo 'saidaMondrian.ppm'\n");
      break;
...
void myDisplay(void)
{
  ...
  float cor[3];
  glBegin(GL_POINTS);
  for (lin = 0; lin < gImgR->nL; lin++)
    for (col = 0; col < gImgR->nC; col++) {
      cor[0] = getPixel(gImgR, lin, col);
      cor[1] = getPixel(gImgG, lin, col);
      cor[2] = getPixel(gImgB, lin, col);
      glColor3fv( cor );
      glVertex2f( col, lin);
    }
  glEnd();
  glFlush();
  printf("\nDigite: \n");
  printf("   't' para trocar uma cor\n");
  printf("   's' para salvar a imagem atual no arquivo saidaMondrian.ppm\n");
  printf("   'q' para encerrar o programa.\n");
}
